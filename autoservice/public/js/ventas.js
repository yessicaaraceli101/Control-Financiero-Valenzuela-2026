...
async function confirmarPagoFinal() {
  ...
  // (Esto solo es visual, NO debe decidir el nombre final en DB)
  formaPagoFinal = {
  ...
  // Fecha default = hoy si viene vacío
  ...
  // ✅ SIEMPRE refrescar caja antes de usar window.cajaActual.id
  if (typeof refrescarCajaAbierta === "function") {
    await refrescarCajaAbierta();
  }
  console.log("CAJA ACTUAL:", window.cajaActual);

  if (formaPagoIdSeleccionada === 1) {
    // por seguridad, refrescamos otra vez
    if (typeof refrescarCajaAbierta === "function") {
      await refrescarCajaAbierta();
    }
    caja_id = Number(window.cajaActual?.id) || null;
  } else {
    caja_id = null;
  }

  // ✅ Si es efectivo y NO hay caja abierta -> modal profesional (sin fondo transparente)
  if (formaPagoIdSeleccionada === 1 && !caja_id) {
    // cerramos tus modales custom si existen
    if (typeof closeModal === "function") closeModal("modalPago");
    if (typeof closeModal === "function") closeModal("modalVenta");

    // y mostramos el modal Bootstrap
    mostrarModalCajaCerrada("Debe abrir la caja antes de realizar una venta en efectivo.");
    console.warn("⚠️ caja_id es null. Revisá /caja/estado y window.cajaActual");
    return;
  }
  ...
  const body = {
    fecha,
    cliente_id,
    caja_id, // null si no es efectivo
    total,
    forma_pago_id: formaPagoIdSeleccionada,
    estado_pago,
    nro_comprobante, // ✅ NUEVO
    items: ventaItems
  };
  ...
  try {
    ...
    if (!res.ok) {
      ...
      // ✅ cerrar modales custom para evitar superposición
      if (typeof closeModal === "function") closeModal("modalPago");
      if (typeof closeModal === "function") closeModal("modalVenta");

      // si el error incluye “caja”, lo mostramos con modal
