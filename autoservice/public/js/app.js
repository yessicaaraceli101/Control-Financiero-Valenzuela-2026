...
// 🟢 ABRIR CAJA (FIX: no revienta si faltan inputs + unifica window.cajaActual)
async function abrirCaja() {
  try {
    const tipoEl = document.getElementById("tipoCaja");
    const fechaEl = document.getElementById("fechaCaja");
    const saldoEl = document.getElementById("saldoInicialCaja"); // puede NO existir

    // ✅ leer valores sin romper
    const tipo = (tipoEl?.value || "Efectivo").trim();
    const fecha = (fechaEl?.value || "").trim();

    // ✅ si no tenés input saldo inicial, queda 0
    ...
    const res = await fetch(`${API}/caja/abrir`, {
      ...
      headers: { "Content-Type": "application/json" },
      credentials: "include",
      body: JSON.stringify({ tipo, fecha, saldo_inicial: saldoInicial })
    });
    ...
    // ✅ Guardar SIEMPRE en window (para que ventas lo vea también)
    if (id) {
      window.cajaActual = data.caja || { id, tipo, fecha, saldo_inicial: saldoInicial };
      ...
      if (!window.cajaActual || !window.cajaActual.id) {
        alert("La caja se abrió, pero no se pudo obtener el ID. Revisá la respuesta de /caja/abrir");
    ...
    if (estadoEl) {
      estadoEl.innerHTML = `🟢 Caja ABIERTA (${tipo}) — ${saldoInicial.toLocaleString("es-PY")} Gs.`;
    }
...
// 🔴 CERRAR CAJA (FIX: usa window.cajaActual y soporta endpoints distintos)
async function cerrarCaja() {
  try {
    ...
    if (!window.cajaActual || !window.cajaActual.id) {
      alert("No hay caja abierta (o falta el ID).");
      return;
    }

    const id = window.cajaActual.id;
...
async function cargarRecaudacionFecha() {
  ...
  try {
    // tu backend acepta dd/mm/yyyy (según tu captura) o y-m-d
    // usamos la misma fecha del input
    ...
    // ✅ VIENE ASÍ: { ok:true, fecha:"YYYY-MM-DD", dia:{...}, mes:{...} }
    const dia = data.dia || {};
    const mes = data.mes || {};

    document.getElementById("dia-efectivo").textContent       = "Gs. " + money(dia.efectivo);
    document.getElementById("dia-transferencia").textContent  = "Gs. " + money(dia.transferencia);
    document.getElementById("dia-total").textContent          = "Gs. " + money(dia.total);

    document.getElementById("mes-efectivo").textContent       = "Gs. " + money(mes.efectivo);
    document.getElementById("mes-transferencia").textContent  = "Gs. " + money(mes.transferencia);
    document.getElementById("mes-total").textContent          = "Gs. " + money(mes.total);
...
async function verificarCaja() {
  try {
    // 1) estado de caja
    const res = await fetch(`${API}/caja/estado`, { credentials: "include" });
    const data = await res.json();

    cajaActual = data?.caja ?? data?.data?.caja ?? null;
    cajaAbierta = !!(cajaActual && cajaActual.id);

    window.cajaActual = cajaActual;
    ...
    const fecha = document.getElementById("fechaCaja")?.value || cajaActual.fecha;
    let diaTotal = 0;
    ...
    try {
      ...
      if (r2.ok && d2?.dia) {
        diaTotal = Number(d2.dia.total || 0);
      }
    } catch (e) {
      console.warn("No se pudo leer /caja/resumen para saldo:", e);
    }

    const saldoInicial = Number(cajaActual.saldo_inicial || 0);
    const saldoReal = saldoInicial + diaTotal;

    el.innerHTML = `🟢 Caja ABIERTA (${cajaActual.tipo || "-"}) — Saldo: Gs. ${money(saldoReal)}`;
...
(function bindCajaEventos(){
  ...
  // evitar duplicar listeners si recargas scripts
  ...
  f.addEventListener("change", () => {
    ...
    // ✅ si estás viendo Formas de pago, refrescar también
    if (location.hash === "#formas-pago") listarFP();
  });

  // también cuando cambias tipo de caja si querés
  const t = document.getElementById("tipoCaja");
...
function confirmLogout(){
  localStorage.removeItem("auth");  // o el nombre real de tu key
  closeLogoutModal();
  window.location.href = "login.html";
}
...
document.addEventListener("click", async (e) => {
  // Abrir caja
  const abrir = e.target.closest("#btnAbrirCaja");
  if (abrir) {
    e.preventDefault();
    if (typeof abrirCaja === "function") return abrirCaja();
    console.error("❌ No existe la función abrirCaja()");
    return;
  }

  // Cerrar caja
  const cerrar = e.target.closest("#btnCerrarCaja");
  if (cerrar) {
    e.preventDefault();
    if (typeof cerrarCaja === "function") return cerrarCaja();
    console.error("❌ No existe la función cerrarCaja()");
    return;
  }
});
