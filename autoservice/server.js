...
app.post("/ventas", async (req, res) => {
  ...
  try {
    ...
    // ✅ Requerir comprobante solo para transferencias / bancos / QR / etc (todo menos efectivo)
    const compStr = (nro_comprobante || "").toString().trim();
    if (fpId !== 1 && !compStr) {
      return res.status(400).json({ ok: false, msg: "Falta nro_comprobante" });
    }

    // ✅ REQUERIR caja abierta SIEMPRE (para cualquier forma de pago)
let caja_id_final = null;

const caja = await pool.query(
  "SELECT id, tipo FROM caja WHERE estado='abierta' ORDER BY id DESC LIMIT 1"
);

if (caja.rows.length === 0) {
  return res.status(400).json({
    ok: false,
    msg: "Debe abrir la caja antes de realizar una venta",
  });
}

caja_id_final = caja.rows[0].id;
    ...
    // - si efectivo => NULL
    // - si no efectivo => string
    const compFinal = fpId === 1 ? null : compStr;

    // ✅ INSERTAR VENTA (fecha SIEMPRE local PY)
...
app.get("/caja/abierta", async (req, res) => {
  try {
    ...
    const q = tipo
      ? "SELECT * FROM caja WHERE estado='abierta' AND tipo=$1 ORDER BY id DESC LIMIT 1"
...
app.post("/caja/cerrar", async (req, res) => {
  try {
    const { saldo_cierre, tipo } = req.body || {};
    const tipoNorm = tipo ? String(tipo).trim().toLowerCase() : null;

    const q = tipoNorm
      ? `UPDATE caja
         SET saldo_cierre=$1, estado='cerrada', cerrado_en=NOW()
         WHERE estado='abierta' AND tipo=$2
         RETURNING *`
      : `UPDATE caja
         SET saldo_cierre=$1, estado='cerrada', cerrado_en=NOW()
    ...
    const params = tipoNorm
      ? [Number(saldo_cierre || 0), tipoNorm]
      : [Number(saldo_cierre || 0)];

    const r = await pool.query(q, params);

    if (r.rowCount === 0) {
      return res.status(400).json({ ok: false, msg: "No hay caja abierta" });
    }

    res.json({ ok: true, caja: r.rows[0] });
  } catch (err) {
    console.error("POST /caja/cerrar", err);
    res.status(500).json({ ok: false, msg: "Error al cerrar caja" });
  }
});
app.get("/caja/estado", async (req, res) => {
  try {
    ...
    console.error("Estado caja:", err);
    res.status(500).json({ abierta: false });
  }
});
...
app.get("/caja/resumen-dia", async (req, res) => {
  try {
    const diaParam = req.query.dia;
    if (!diaParam) return res.status(400).json({ ok: false, msg: "Falta dia" });
    ...
    return res.json({ ok: true, dia: ymd, ...dia.rows[0] });
  } catch (err) {
...
app.get("/caja/resumen-mes", async (req, res) => {
  try {
    ...
    if (!mesParam) return res.status(400).json({ ok: false, msg: "Falta mes" });
    ...
    return res.json({ ok: true, mes: ymd, ...mes.rows[0] });
  } catch (err) {
...
// Opcional: un endpoint único para que el front llame 1 sola vez
// GET /caja/resumen?fecha=11/02/2026
app.get("/caja/resumen", async (req, res) => {
  try {
    ...
    const ymd = toISODate(fechaParam); // YYYY-MM-DD
